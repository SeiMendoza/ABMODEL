...
@section('productos')
    @foreach($products as $pro)
        @if ($pro->disponible >= 1)
           <div class="col " style="padding: 0px; margin:0px;">
                <form action="{{route('cart.create')}}" method="post" id="crear" name="crear" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="d-flex justify-content-center mb-1">
                        <button class="card btn btnCard col" id="btn" onclick="enviar()"
                            data-id="{{$pro->id}}" style="padding: 0px; width:215px; height:200px; margin:0; border-radius:0%;
    ...
    <script>
        function enviar() {
            var formul = document.getElementById("crear");
            formul.submit();
        }
    </script>
@endsection
    ...
    <div class="page-wrapper bg-red p-t-170 p-b-100 font-robo">
        ...
        <div class="wrapper wrapper--w960" >
            <div class="card card-2">
                ...
                <div class="card-body">
                    <h2 class="title">Editar {{ $PlatillosyBebidas -> nombre}}</h2>
                    <form method="post"  action="{{ route('platobebida.update', ['id' => $PlatillosyBebidas -> id ]) }}" enctype="multipart/form-data">
                        ...
                        <div style="width:200px;float:left">
                            ...
                            value="{{ $PlatillosyBebidas -> imagen }}" style="color: white;width: 200px;" >
                            @error('imagen')
                        ...
                        <div style="margin-left:2%;float:left;width:35%">
                            <div class="rs-select2 js-select-simple select--no-search">
                                <select name="tipo" id="tipo" required onchange="producto();quitarerror()" value="{{ $PlatillosyBebidas -> tipo }}">
                                @if(old('tipo'))
                                    @if(old('tipo') === 2 )
...
<br>
                            ...
                            maxlength="25" required onkeypress="quitarerror()" value="{{ $PlatillosyBebidas -> nombre }}">
                            @error('nombre')
                                <strong class="menerr" style="color:red">{{ $message }}</strong>
                            @enderror
<br><br>
                            <textarea class="textarea--style-2" type="text" placeholder="Descripción" name="descripcion"  maxlength="100"
                            required onkeypress="quitarerror()">{{ $PlatillosyBebidas -> descripcion }}</textarea>
                            @error('descripcion')
                                <strong class="menerr" style="color:red">{{ $message }}</strong>
                            @enderror
<br><br>
                        ...
                        <div style="margin-left:2%;float:left;width:35%">
                        <div class="rs-select2 js-select-simple select--no-search">
                                <select name="tamanio" required onchange="quitarerror()" value="{{ $PlatillosyBebidas -> tamanio }}">
                                @if(old('tamanio'))
...
<br>
                            ...
                            value="{{ $PlatillosyBebidas -> precio }}" onkeypress="quitarerror()"
                            ...
                            <div id="bebida" style="display:none">
                                ...
                                value="{{ $PlatillosyBebidas -> cantidad }}" onkeypress="quitarerror()"
                            ...
                            <div id="comida" style="display:none">
                                ...
                                value="{{ $PlatillosyBebidas -> disponible }}" onkeypress="quitarerror()"
                                onkeydown="javascript: return event.keyCode == 69 ? false : true" min="1" max="1000" disabled>
                                @error('disponible')
                                    <strong class="menerr" class="menerr" style="color:red">{{ $message }}</strong>
                                @enderror
                            ...
                            <div style="float:right">
                                <button type="submit" href="{{ route('admonRestaurante')  }}" class="btn btn-success">Guardar</button>
                                <button type="button" onclick="cancelar('/')" href="{{ route('admonRestaurante')  }}" class="btn btn-warning">Cancelar</button>
                            </div>
                        </div>

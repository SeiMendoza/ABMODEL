                ...
                @forelse($listaUs as $u => $l)
                    <tr style=" height:46px">
                        ...
                        <td style="text-align: center;">
                            <a href="{{ route('usuarios.editar', ['id' => $l->id]) }}"><i
                                    class="fa fa-edit text-success"></i></a>
                        </td>

                        <td scope="col" style="text-align: center;">
                            @if (Auth::user()->id !== $l->id)
                                <i data-bs-toggle="modal" data-bs-target="#staticBackdropE{{ $l->id }}"
                                    class="fa-solid fa-trash-can text-danger" style="color:crimson"></i>
                            @endif

                            @if (Auth::user()->id !== $l->id)
                                <form action="{{ route('usuarios.destroy', ['id' => $l->id]) }}" method="post"
                                    enctype="multipart/form-data">
                                    @method('delete')
                                    @csrf
                                    <div class="modal fade" id="staticBackdropE{{ $l->id }}"
